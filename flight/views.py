...
def check(request):
    ...
    for booking in bookings:
        ...
        flight.arrive_time1 = addzero(
            str(flight.arrive_time.hour)) + ":" + addzero(str(flight.arrive_time.minute))
        flight.leave_time1 = addzero(
            str(flight.leave_time.hour)) + ":" + addzero(str(flight.leave_time.minute))
...
def result(request):
    ...
    if (request.POST.get("restart") != '2'):
        pagecur = 1
    print(request.POST.get("restart"))
    if transfer == 1:
        if company1 == 0:
            flights = Flight.objects.filter(
                leave_city=leave_city, arrive_city=arrive_city)
        else:
            flights = Flight.objects.filter(
                leave_city=leave_city, arrive_city=arrive_city, flight_company=companies[company1 - 1])
        available_flights = []
        for flight in flights:
            if flight.leave_time.date() == leave_date.date():
                if start_price.strip() != "" and flight.ticket_price < float(start_price):
                    continue
                if end_price.strip() != "" and flight.ticket_price > float(end_price):
                    continue
                cope(flight)
                available_flights.append(flight)
        if sort_way == 1:
            ...
            available_flights.sort(
                key=lambda x: x.arrive_time-x.leave_time, reverse=rev)
        pagenum = 6
        pagetot = (len(available_flights) - 1) // pagenum + 1
        available_flights = available_flights[(
            pagecur - 1) * pagenum: pagecur * pagenum]
    else:
        available_flights = []
        for middle_city in cities:
            if company1 == 0:
                flights = Flight.objects.filter(
                    leave_city=leave_city, arrive_city=middle_city)
            else:
                flights = Flight.objects.filter(
                    leave_city=leave_city, arrive_city=middle_city, flight_company=companies[company1 - 1])
            for flight_1 in flights:
                if flight_1.leave_time.date() != leave_date.date():
                    continue
                arrive_time = flight_1.arrive_time + timedelta(hours=2)
                flights_2 = Flight.objects.filter(
                    leave_city=middle_city, arrive_city=arrive_city, leave_airport=flight_1.arrive_airport, leave_time__gte=arrive_time)
                for flight_2 in flights_2:
                    if flight_2.leave_time > arrive_time + timedelta(hours=5):
                        continue
                    flight = {}
                    flight["flight_1"] = flight_1
                    flight["flight_2"] = flight_2
                    cope(flight_1)
                    cope(flight_2)
                    used_time = flight_2.arrive_time - flight_1.arrive_time
                    flight["time"] = used_time
                    flight["price"] = flight_1.ticket_price1 + \
                        flight_2.ticket_price1
                    if start_price.strip() != "" and flight["price"] < float(start_price):
                        continue
                    if end_price.strip() != "" and flight["price"] > float(end_price):
                        continue
                    available_flights.append(flight)
        if sort_way == 1:
            available_flights.sort(key=lambda x: x["flight_1"].leave_time, reverse=rev)
        elif sort_way == 2:
            available_flights.sort(key=lambda x: x["flight_2"].arrive_time, reverse=rev)
        elif sort_way == 3:
            available_flights.sort(key=lambda x: x["price"], reverse=rev)
        elif sort_way == 4:
            available_flights.sort(key=lambda x: x["ticket_per"], reverse=rev)
        elif sort_way == 5:
            available_flights.sort(
                key=lambda x: x["time"], reverse=rev)
        pagenum = 6
        pagetot = (len(available_flights) - 1) // pagenum + 1
        available_flights = available_flights[(
            pagecur - 1) * pagenum: pagecur * pagenum]  
        pagestart = (pagecur - 1) * pagenum
...
def result_booking(request):
    ...
    leave_time = datetime.strptime(leave_time, " %H:%M")
    user = request.user
    customer = Customer.objects.get(user=user)
    name = customer.name
    
    flight_id1 = request.POST.get("flight_id1")
    if flight_id1 is not None:
        leave_date1 = request.POST.get("leave_date")
        leave_time1 = request.POST.get("leave_time1")
        leave_date1 = datetime.strptime(leave_date1, "%Y 年 %m 月 %d 日")
        leave_time1 = datetime.strptime(leave_time1, " %H:%M")
        if leave_time1 < leave_time:
            leave_date1 += timedelta(days=1)
    ...
    for fly in flights:
        if fly.leave_time.date() == leave_date.date():
            flight = fly
            break
    booking = Booking(flight=flight, customer=customer,
                      booking_price=flight.ticket_price)
    if (flight.ticket_bought >= flight.ticket_number):
        return HttpResponse("2")
    flight.ticket_bought += 1

    if flight_id1 is not None:    
        flights = Flight.objects.filter(flight_id=flight_id1)
        for fly in flights:
            if fly.leave_time.date() == leave_date1.date():
                flight1 = fly
                break
        booking1 = Booking(flight=flight1, customer=customer,
                      booking_price=flight.ticket_price)
        if (flight1.ticket_bought >= flight1.ticket_number):
            return HttpResponse("2")
        flight1.ticket_bought += 1
...
def check_booking(request):
    ...
    booking = Booking.objects.filter(flight=flight, customer=customer)[0]
    flight.ticket_bought -= 1
    flight.save()
    booking.delete()
...
def insert_data(request):
    ...
    for flight in flights:
        ...
        for fly in content:
            ...
            flight_company, created = Company.objects.get_or_create(
                name=flight_company)
            flight_id = fly["id"]
            ...
            leave_airport, created = Airport.objects.get_or_create(
                name=leave_airport)
            arrive_airport, created = Airport.objects.get_or_create(
                name=arrive_airport)
